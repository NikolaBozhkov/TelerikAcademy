...
class MultiSnakeUpdate3
{
    ...
    static void InitialiseSnake(Random foodGenerator, Queue<Position> snakeBody, int snakeNumber)
    {
        for (int i = 0; i <= 6; i++)
        {
    ...
    StartGame:
        ...
        //Initialise Snake
        InitialiseSnake(foodGenerator, firstSnake, snake1Start);

        //Initialise Second Snake
        InitialiseSnake(foodGenerator, secondSnake, snake2Start);
        ...
        while (true)
        {
            ...
            if (newFirstHead.X >= Console.WindowWidth ||
                ...
                newFirstHead.Y < 0 ||
                firstSnake.Contains(newFirstHead)) //||firstSnake.Contains(newSecondHead))
            {
                // Game is over
                Console.SetCursorPosition(0, 0);
                Console.ForegroundColor = ConsoleColor.Gray;
                Console.WriteLine("Game over!!! Player 2 wins !!! with {0} points,\nPlayer 1 has {1} points", secondPoints, firstPoints);
            ...
            if (newSecondHead.X >= Console.WindowWidth ||
                ...
                newSecondHead.Y < 0 ||
                secondSnake.Contains(newSecondHead)) //||secondSnake.Contains(newFirstHead))
            {
                // Game is over
                Console.SetCursorPosition(0, 0);
                Console.ForegroundColor = ConsoleColor.Gray;
                Console.WriteLine("Game over!!! Player 1 wins !!! with {0} points,\nPlayer 2 has {1} points", firstPoints, secondPoints);
                Console.Write("Do you want to play again? (y/n): ");
                char restart = char.Parse(Console.ReadLine());
                if (restart == 'y' || restart == 'Y')
                {
            ...
            {
                if (prevFirstHead.X != prevSecondHead.X &&
                prevFirstHead.Y != prevSecondHead.Y || newFirstHead.X != newSecondHead.X && newFirstHead.Y != newSecondHead.Y)
                {
                    Console.SetCursorPosition(0, 0);
