<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Home.aspx.cs" Inherits="DogeSocialNetwork.Home" %>

<!DOCTYPE html>

<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
    <title>Doge Social Network - Home</title>
</head>
<body BGCOLOR="#58D3F7">
    <h1>Doge Social Network</h1>
    <ul type="disc">
        <li>
            <a title="Home" href="Home.aspx">Home</a>
        </li>
        <li>
            <a title="Profile" href="Profile.aspx">Profile</a>
        </li>
        <li>
            <a title="Friends" href="Friends.aspx">Friends</a>
        </li>
    </ul>
    <div>
        <hr />
        <h2>Welcome to the Doge Social Network</h2>
        <hr />
    </div>
    <p>Here you can find friends or stalk strangers.</p>
    <p>The Doge Social Network does not take any responsibility for the information found.</p>
</body>
</html>
