...
namespace TradeAndTravel
{
    public class InteractionManager
    {
        ...
        private void HandleCraftInteraction(string[] commandWords, Person actor)
        {
            ...
            {
                if (inventory.Exists(i => i.ItemType == ItemType.Iron))
                {
            ...
            {
                if (inventory.Exists(i=> i.ItemType == ItemType.Iron) &&
                    inventory.Exists(i => i.ItemType == ItemType.Wood))
                {
        ...
        private void HandleGatherInteraction(string[] commandWords, Person actor)
        {
            ...
            {
                if (inventory.Exists(i => i.ItemType == ItemType.Weapon))
                {
                    item = new Wood(commandWords[2]);
                }
            }
            else if (actor.Location.LocationType == LocationType.Mine)
            {
                if (inventory.Exists(i => i.ItemType == ItemType.Armor))
                {
                    item = new Iron(commandWords[2]);
                }
            }
